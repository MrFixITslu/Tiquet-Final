...
export function registerOAuthRoutes(app) {
    ...
    app.post('/api/auth/facebook', async (req, res) => {
        ...
        if (!FACEBOOK_SECRET) {
            return res.status(503).json({ error: 'Facebook login is not fully configured on this server' });
        }

        try {
            // SECURITY: verify the token was actually issued to THIS app before trusting
            // it. Without this check, any valid Facebook access token — including one
            // issued to a completely different, unrelated app — would be accepted here,
            // letting an attacker log in as whichever user they can get a token for
            // (token-confusion / token-substitution vulnerability).
            const appAccessToken = `${FACEBOOK_ID}|${FACEBOOK_SECRET}`;
            ...
            if (!tokenInfo || !tokenInfo.is_valid || String(tokenInfo.app_id) !== String(FACEBOOK_ID)) {
                logger.warn('Facebook OAuth rejected: token not valid for this app', {
                    appId: tokenInfo?.app_id, expected: FACEBOOK_ID, isValid: tokenInfo?.is_valid,
                });
            ...
            // Extra parity check: the user id returned by /me must match the id the
            // token was debugged against, so a mismatched/spoofed token can't slip through.
            if (String(fbData.id) !== String(tokenInfo.user_id)) {
                logger.warn('Facebook OAuth rejected: /me id does not match debug_token user_id');
                return res.status(401).json({ error: 'Invalid or expired Facebook credential' });
            }
