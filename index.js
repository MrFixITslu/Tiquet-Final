...
app.post("/api/jobs", authenticateToken, (req, res) => {
    const { id: reqId, title, client, description, status, createdAt, dueDate, amount, priority, invoiceNotes, assignedTo, clientEmail, tags, activityLog, depositPaid, lineItems, deliverables, timerStartedAt, stageAssignments, timeLogs } = sanitizeObject(req.body);
    const id = reqId || uuidv4();
    const secureToken = uuidv4();

    if (clientEmail && !isValidEmail(clientEmail)) {
        return badRequest(res, "Invalid client email format");
    }
...
app.put("/api/jobs/:id", authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { title, client, description, status, dueDate, amount, priority, invoiceNotes, assignedTo, clientEmail, tags, activityLog, depositPaid, quoteApproved, lineItems, deliverables, timerStartedAt, stageAssignments, timeLogs } = sanitizeObject(req.body);
    ...
    try {
        ...
        // Use finalStatus (not the raw request field) so the email reflects the
        // real, post-automation status — e.g. an auto-advance from 'request' to
        // 'estimation' still notifies the client, and always with the correct label.
        const effectiveStatusChanged = finalStatus !== existingJob.status;

        if (effectiveStatusChanged && recipientEmail && token) {
            sendStatusUpdate(recipientEmail, jobTitle, finalStatus, token)
