...
export function ClientPortal({
    ...
    // Real-time chat over WebSocket. The portal token itself is the credential —
    // the server validates it against the job's secureToken before subscribing
    // this connection to that job's message room.
    React.useEffect(() => {
        if (!token) return;
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="mb-8">
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 mb-2">{job.title}</h1>
                        ...
                        <div className="border border-slate-200 p-6 rounded-2xl mb-6 bg-white shadow-sm">
                            ...
                            <div className="space-y-3">
                                {job.deliverables.map(d => (
                                    <div key={d.id} className="flex items-center justify-between gap-3 p-4 bg-slate-50 border border-slate-100 rounded-xl">
                                        <div className="flex items-center gap-3 min-w-0">
                                            {d.type === 'digital' ? <Download className="w-5 h-5 text-slate-400 shrink-0" /> : <Truck className="w-5 h-5 text-slate-400 shrink-0" />}
                                            <div className="min-w-0">
                                                <p className="font-bold text-slate-900 text-sm truncate">{d.title || "Unnamed Asset"}</p>
                                        ...
                                        {d.type === 'digital' ? (
                                            <button 
                                                ...
                                                className={`text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-all shadow-sm shrink-0 ${job.status === "paid" ? "bg-red-500 hover:bg-red-600 text-white shadow-red-200" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}
                                            >
            ...
            {/* Floating Chat */}
            <div className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end transition-all duration-300 ${isChatOpen ? 'w-[calc(100vw-2rem)] max-w-80 h-[70vh] max-h-[450px]' : 'w-14 h-14'}`}>
                {isChatOpen ? (
